#!/usr/bin/env python3
"""
MIDI-TSV v0.1: bidirectional conversion between MIDI files and MIDI-TSV text.
...
MIN_TICK_SCALE = 2
MAX_TICK_SCALE = 20
TARGET_TICK_SCALE_MS = 10  # 1 macro-tick ≈ 10ms at dominant tempo
...
def select_tick_scale(tpq: int, tempos: list[dict]) -> int:
    """Auto-select tick_scale so 1 macro-tick ≈ 10ms at the dominant tempo,
...
def _find_slice_local_start(notes, pedals, slice_, slices) -> int:
    first = float("inf")
...
def midi_to_tsv(data: bytes, source: str = "unknown.mid") -> str:
    ...
    for track_idx, events in enumerate(raw_tracks):
        ...
        for evt in events:
            ...
            if etype == "meta":
                ...
                ch = evt["channel"]
                if track_id not in track_channels:
                    track_channels[track_id] = ch
                key = f"{ch}:{evt['note']}"
                open_notes[key].append({
                    "track_id": track_id, "t": tick, "dur": 0,
                    "pitch": evt["note"], "vel": evt["velocity"], "channel": ch,
                })
            elif etype == "note_off":
                ch = evt["channel"]
                if track_id not in track_channels:
                ...
                if not open_notes[key]:
                    del open_notes[key]
            elif etype == "control_change" and evt["controller"] == 64:
                ch = evt["channel"]
                if track_id not in track_channels:
                    track_channels[track_id] = ch
                pedals.append({"track_id": track_id, "t": tick, "val": evt["value"], "channel": ch})
    ...
    # Sort
    notes.sort(key=lambda n: (n["track_id"], n["t"]))
    pedals.sort(key=lambda p: (p["track_id"], p["t"]))

    tick_scale = select_tick_scale(tpq, tempos)
    slices = create_slices(notes, pedals, end_tick, tpq, tempos, tick_scale)
    all_track_ids = sorted(set(n["track_id"] for n in notes) | set(p["track_id"] for p in pedals))
    ...
    lines: list[str] = [
        ...
        f"# tpq={tpq}",
        "# pitch=abc-absolute",
        f"# voice_map={','.join(f'T{id}:V{id}' for id in all_track_ids)}",
    ]

    if track_channels:
        ch_items = sorted(track_channels.items())
        lines.append(f"# track_channel={','.join(f'T{k}:V' for k, v in ch_items)}".replace(
            "T:", "T").replace("V:", f"T:")
        )
        # Rebuild properly
        lines[-1] = f"# track_channel={','.join(f'T{k}:{v}' for k, v in ch_items)}"
    ...
    for sl in slices:
        local_start = _find_slice_local_start(notes, pedals, sl, slices)
        ...
        for track_id in all_track_ids:
            ...
            for n in notes:
                if n["track_id"] == track_id and n["t"] >= local_start and (n["t"] < sl["end"] or is_last):
                    records.append({
                        "t": n["t"],
                        "order": 1,
                        "line": f"{midi_pitch_to_abc(n['pitch'])}\t{scale_tick(n['t'] - local_start, tick_scale)}\t{scale_duration(n['dur'], tick_scale)}\t{n['vel']}",
                    })
            for p in pedals:
                if p["track_id"] == track_id and p["t"] >= local_start and (p["t"] < sl["end"] or is_last):
                    records.append({
                        "t": p["t"],
                        "order": 0,
                        "line": f"P\t{scale_tick(p['t'] - local_start, tick_scale)}\t{p['val']}",
                    })
            records.sort(key=lambda r: (r["t"], r["order"]))
            if not records:
                continue
            lines.append(f"T\t{track_id}")
            for r in records:
                lines.append(r["line"])
            lines.append("")
